...
int pos = 0;
// offset variables for calibration for each speed controller
          ...
          if (current_line.length() == 0)
          {
            ...
            client.println("\
              ...
              <body>\
                  ...
                  <div class='slidecontainer'>\
                      <p>Forwards: <span id='fValueDisplay'></span></p>\
                      <input type='range' min='0' max='500' value='0' class='slider' id='fSlider' style='color: #0000FF'>\
                  </div>\
                  ...
                  <script>\
                      ...
                      fSlider.oninput = function() {\
                          fValueDisplay.innerHTML = this.value;\
                          \
                          let localIP = window.location.host;\
                          let address = 'http://' + localIP + '/R' + lpad(this.value, 3);\
                          let xmlHttp = new XMLHttpRequest();\
                      ...
                      stopButton.onclick = function() {\
                          ...
                          hoverSlider.value = '0';\
                          lValueDisplay.innerHTML = '0';\
                          lSlider.value = '0';\
                          rValueDisplay.innerHTML = '0';\
                          rSlider.value = '0';\
                          \
                          let localIP = window.location.host;\
          ...
          if (poss_get_req == "GET /")
          {
            ...
            if (is_number(poss_slider_value))
            {
              ...
              else if (poss_slider_param[0] == 'L')
              {
                ...
                motor_R.writeMicroseconds((slider_value ) + esc_offset); //* 0.5
              }
              else if (poss_slider_param[0] == 'R')
              {
                motor_R.writeMicroseconds((slider_value * 2) + esc_offset);
                motor_L.writeMicroseconds((slider_value * 1.6) + esc_offset);
              }
              else if (poss_slider_param[0] == 'F')
              {
                motor_R.writeMicroseconds((slider_value) + esc_offset);
                motor_L.writeMicroseconds((slider_value * 2) + esc_offset);
              }
