...
void setup() {
  ...
  // sets 3-phase BRUSHLESS motors to zero speed
  ...
  if (client)
  {
    ...
    while (client.connected())
    {
      if (client.available())
      {
            ...
            client.println("\
              ...
              <body>\
                  ...
                  <div class='slidecontainer'>\
                      ...
                      <input type='range' min='0' max='180' value='90' class='slider' id='directionSlider' style='color: #FF0000'>\
                  </div>\
                  ...
                  <script>\
                      ...
                      forwardsButton.onclick = function() {\
                          directionValueDisplay.innerHTML = '90';\
        ...
        // the request should be in the format of GET /H000, GET /F000, GET /A000, GET /C000
        ...
        if (curr_line_length == 9)
        {
          ...
          if (poss_get_req == "GET /")
          {
            ...
            if (is_number(poss_slider_value))
            {
              ...
              {
                if (prev_value < slider_value) {
                  servo_CW(prev_value, slider_value);
                }
                else if (prev_value > slider_value) {
                  servo_ACW(prev_value, slider_value);
                }
              }
...
void servo_CW(int prev_value, int slider_value) {
  for (int dutyCycle = prev_value; dutyCycle <= slider_value; dutyCycle++) {
    servo_L.write(dutyCycle);
    servo_R.write(dutyCycle);
    delay(20);
  }
  prev_value = slider_value;
}

void servo_ACW(int prev_value, int slider_value) {
  for (int dutyCycle = prev_value; dutyCycle >= slider_value; dutyCycle--) {
    servo_L.write(dutyCycle);
    servo_R.write(dutyCycle);
    delay(20);
